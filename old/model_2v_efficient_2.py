...
class VariationalGraphEncoder(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.conv1 = ChebConv(in_channels, 2*out_channels, 3, normalization='rw') #5
...
class PerturbModel(torch.nn.Module):
    def __init__(self, num_node_features, n_channels, edge_index, ctrl_gex, device):
        ...
        # encoder layers
        self.encoder = VariationalGraphEncoder(num_node_features + n_channels, n_channels) # gex + embedding of the perturbation
    ...
    def forward(self, edge_attr, pert):
        ...
        pert = pert.reshape(-1) # shape [batch]
        p = self.pert_embedding(pert) #shape: [batch_size, n_channels]
        p_tiled = p.repeat_interleave(self.num_genes, dim=0)
        ...
        gnn_input = torch.cat([x, p_tiled], dim=1) # shape: [B*N, 1 + n_channels]
        ...
        mu, logstd = self.encoder(gnn_input, edge_index_batch, edge_attr) # shape [B*N,C]

        mu_agg = mu.view(batch_size, self.num_genes, -1).mean(dim=1)     # [B, C]
        logstd_agg = logstd.view(batch_size, self.num_genes, -1).mean(dim=1) # [B, C]

        # Store the aggregated tensors for a graph-level KL loss
        self.last_mu = mu_agg        
        self.last_logstd = logstd_agg
        
        # self.last_mu = mu        # Store for loss calculation
        # self.last_logstd = logstd  # Store for loss calculation

        z = self.reparametrize(mu_agg, logstd_agg)
