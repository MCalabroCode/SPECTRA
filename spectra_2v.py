...
class PerturbModel(torch.nn.Module):
    ...
    def generative_prediction(self, ko_gene_idx, n_samples=1):
        ...
        with torch.no_grad():
            ...
            predicted_expression = self.gex_decoder(z_perturbed, edge_index_batch)
...
def compute_mmd(x, y, kernel_mul=2.0, kernel_num=5, fix_sigma=None):
    ...
    L2_distance = torch.cdist(total, total, p=2)**2 #NOTE: NEW
...
def train_step_perturb_model(model, data, device, alpha=1., beta=1., mmd_gamma=1.):
    x_ = model(data) #[BxN,1]
    ...
    pert_indices = perturbation.int().argmax(dim=1)#TODO: fix this, doesn't work with multiple perturbations; pass directly the string of perturb.
    ...
    # unique_perts = torch.unique(weight_lookup_indices)
    # mmd_count = 0
    # for p_idx in unique_perts: # for each unique perturbation in this batch...
    #     mask = (weight_lookup_indices == p_idx)
    #     if mask.sum() > 1: # We need at least 2 samples to meaningfully compute distribution statistics
    #         # select all the k elements labeled with perturb p_idx
    #         group_pred = x_pred[mask] # [k, num_nodes]
    #         group_true = x_true[mask] # [k, num_nodes]       
    #         # Compute MMD for this group
    #         loss_mmd += compute_mmd(group_pred, group_true)
    #         mmd_count += 1
    # # TODO: not sure this is ideal, not present in the MORPH code, but it's also true that here the batches are not separated per perturb.
    # if mmd_count > 0:
    #     loss_mmd = loss_mmd / mmd_count

    # mmd 
    #loss_mmd = compute_mmd(x_true, x_pred)
    z = model.last_mu # [B*N, Channels]
    # true_samples = torch.randn_like(z)
    # loss_mmd = compute_mmd(z, true_samples)

    # this is like the above but selecting only a subset of variables to not make the HPC explode
    max_mmd_samples = 500
    if z.size(0) > max_mmd_samples:
        perm = torch.randperm(z.size(0))
        idx = perm[:max_mmd_samples]
        z_subset = z[idx]
    else:
        z_subset = z
    true_samples = torch.randn_like(z_subset)
    loss_mmd = compute_mmd(z_subset, true_samples)
...
def test_perturb_model(model, loader, device, wmse=True):
    ...
    for i, data in enumerate(tqdm(loader, desc='testing...')):
        ...
        if wmse:
            ...
            is_control = perturbation.sum(dim=1) == 0
            is_control = is_control.to(device)
            pert_indices = perturbation.int().argmax(dim=1)#TODO: fix this
            pert_indices = pert_indices.to(device)
            weight_lookup_indices = torch.where(is_control, 
                                                torch.tensor(model.num_nodes, device=device), 
                                                pert_indices)
...
def train(model, train_loader, test_loader, lr, n_epochs, device, live_plot):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for (i,batch) in enumerate(tqdm(train_loader, desc=f'training at epoch {epoch}')):
            ...
            loss, feat_loss, kl_loss = train_step_perturb_model(model, batch, model.device, alpha=1.0, beta=1.)
